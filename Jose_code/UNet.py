...
class UNetD(nn.Module):
    def __init__(self, nin, nD):
        ...
        self.convs = nn.Sequential(convBatch(nin, nD),
                                   convBatch(nD, nD),
                                   ...
                                   convBatch(nD * 16, nD * 16, stride=2),
                                   convBatch(nD * 16, nD * 16))
        self.final = nn.Sequential(convBatch(nD * 16, nD * 16),
...
class UNetDeepSupervision(nn.Module):
    def __init__(self, nin, nG, nout):
        ...
        self.out_0125 = nn.Conv2d(nG * 16,nout, kernel_size=3, stride=1, padding=1) # Try to add another conv (nG * 8, nG * 8, kernel_size = 1, stride =1, padding=2)
        self.out_025  = nn.Conv2d(nG * 8,nout, kernel_size=3, stride=1, padding=1)
        self.out_05   = nn.Conv2d(nG * 4,nout, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, input):
        ...
        y = self.deconv0(bridge)
        y0 = self.deconv1(self.conv4(torch.cat((y, x3), dim=1)))
...
class UNetG_Dilated(nn.Module):
    def __init__(self, nin, nG, nout):
        super(UNetG_Dilated, self).__init__()
        self.conv0 = nn.Sequential(convBatch(nin, nG,stride=1, dilation=2, padding=2),
                                   convBatch(nG, nG))
        self.conv1 = nn.Sequential(convBatch(nG * 1, nG * 2, stride=2, dilation=2, padding=2),
                                   convBatch(nG * 2, nG * 2))
        self.conv2 = nn.Sequential(convBatch(nG * 2, nG * 4, stride=2, dilation=2, padding=2),
                                   convBatch(nG * 4, nG * 4))
        self.conv3 = nn.Sequential(convBatch(nG * 4, nG * 8, stride=2, dilation=2, padding=2),
                                   convBatch(nG * 8, nG * 8))
        self.bridge = nn.Sequential(convBatch(nG * 8, nG * 16, stride=2, dilation=2, padding=2),
                                    residualConv(nG * 16, nG * 16),
                                    convBatch(nG * 16, nG * 16))
...
class UNetDisc(nn.Module):
    ...
    def forward(self, input):
        ...
        bridgeflatten = bridge.view(bridge.shape[0],
                                    int(bridge.numel() / bridge.shape[0]))  # Flatten to (batchSize, numElemsPerSample)
